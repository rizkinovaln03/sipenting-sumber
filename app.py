    ...
    with col_form:
        ...
        with st.form("form_skrining"):
            ...
            with st.expander("🦠 Form Skoring TB IDAI (Buka jika ada indikasi)"):
                ...
                # --- UPDATE TB SKORING SESUAI GAMBAR DARI USER (HANYA 7 PARAMETER) ---
                tb_1 = st.selectbox("1. Kontak dengan Pasien TB", ["Tidak jelas (0)", "Laporan keluarga, BTA (-) (1)", "BTA (+) (3)"])
                tb_2 = st.selectbox("2. Uji Tuberkulin (Mantoux)", ["Negatif (0)", "Positif (≥10mm atau ≥5mm imunokompromais) (3)"])
                tb_3 = st.selectbox("3. Status Gizi (BB/TB atau BB/U)", ["Normal (0)", "Gizi kurang (BB/TB <90% atau BB/U <80%) (2)", "Gizi buruk (BB/TB <70% atau BB/U <60%) (3)"])
                tb_4 = st.selectbox("4. Gejala Klinis (Demam ≥2 minggu / Batuk ≥3 minggu)", ["Tidak ada (0)", "Ada gejala (1)"])
                tb_5 = st.selectbox("5. Pembesaran Kelenjar Limfe", ["Tidak ada (0)", "Ada (≥1 cm, >1 KGB, tidak nyeri) (1)"])
                tb_6 = st.selectbox("6. Pembengkakan Tulang/Sendi", ["Tidak ada (0)", "Ada (1)"])
                tb_7 = st.selectbox("7. Foto Toraks", ["Normal / kelainan tidak jelas (0)", "Gambaran sugestif TB (1)"])
        ...
        if hitung_ditekan:
            # Algoritma ekstraksi kebal error (Cuma menjumlahkan 7 tb_x sesuai tabel di atas)
            st.session_state.skor_tb_input = sum([ekstrak_skor(x) for x in [tb_1, tb_2, tb_3, tb_4, tb_5, tb_6, tb_7]])
